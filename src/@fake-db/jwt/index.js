...
mock.onPost('/jwt/register').reply(request => {
  const regData = JSON.parse(request.data)

  console.log('regData', regData)

  // If not any of data is missing return 400
  if (!(regData.email && regData.password)) return [400]

  console.log('s1')

  const isEmailAlreadyInUse = data.users.find(user => user.email === regData.email)

  console.log('s2')

  const error = {
    password: !regData.password ? ['Please enter password'] : null,
    email: (() => {
      if (!regData.email) return ['Please enter your email.']
